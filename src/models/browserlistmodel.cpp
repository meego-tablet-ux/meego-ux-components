...
#define CHROMIUM_BROWSER_PATH "/usr/bin/meego-tablet-browser"
...
BrowserItem::BrowserItem(int id, const QString& url, const QString& title, 
...
m_type(type), m_sorttype(SortByDefault), m_limit(20), m_iface(NULL),m_browserIsLaunching(false)
{
	...
	if(m_type == ListofRecentVisited)
	{
		...
		connect(m_iface, SIGNAL(urlVisited(uint, const QString&, const QString&, const QString&)), 
			this, SLOT(urlVisited(uint, const QString&, const QString&, const QString&)));
		connect(m_iface, SIGNAL(urlRemoved(const QString&)), this, SLOT(urlRemoved(const QString&)));
	}

	if(m_type == ListofBookmarks)
	{
		connect(m_iface, SIGNAL(bookmarkUpdated(uint, const QString&, const QString&, const QString&)), 
			this, SLOT(bookmarkUpdated(uint, const QString&, const QString&, const QString&)));
		connect(m_iface, SIGNAL(bookmarkRemoved(uint)), this, SLOT(bookmarkRemoved(uint)));
	}
...
bool BrowserItemListModel::loadDataSource()
{
    ...
    {
        stmt = QString("SELECT urls.id, urls.url, urls.title, favicons.url,last_visit_time FROM urls, favicons "
                       "WHERE urls.favicon_id > 0 and urls.favicon_id=favicons.id "
                       "UNION "
                       "SELECT urls.id, urls.url, urls.title, '', last_visit_time FROM urls, favicons "
                       "WHERE urls.favicon_id=0 "
                       "ORDER BY last_visit_time DESC LIMIT %1").arg(m_limit);

    } else if (m_type == ListofBookmarks) // load bookmark
    {
        stmt = QString("SELECT bookmarks.id, bookmarks.url, bookmarks.title, favicons.url, dateAdded FROM bookmarks, favicons "
                       "WHERE bookmarks.favicon_id>0 and bookmarks.favicon_id=favicons.id "
                       "UNION "
                       "SELECT bookmarks.id, bookmarks.url, bookmarks.title, '', dateAdded FROM bookmarks, favicons "
                       "WHERE bookmarks.favicon_id=0 "
                       "ORDER BY dateAdded DESC LIMIT %1").arg(m_limit);

    } else if (m_type == ListofTabs) // tab being opened in browser
    {
        // TODO:
        stmt = QString("SELECT tab_id, current_tabs.url, current_tabs.title, favicon_url FROM "
                       "(SELECT urls.url as vURL, favicons.url as favicon_url FROM urls, favicons "
                       "WHERE urls.favicon_id > 0 and urls.favicon_id=favicons.id "
                       "UNION "
                       "SELECT urls.url as vURL, urls.title, '' as favicon_url, last_visit_time FROM urls, favicons "
                       "WHERE urls.favicon_id=0), "
                       "current_tabs WHERE current_tabs.url=vURL LIMIT %1").arg(m_limit);
    } else
    {
...
void BrowserItemListModel::setType(ModelType type)
{
	...
	if(m_type == ListofRecentVisited)
	{
		disconnect(m_iface, SIGNAL(urlVisited(uint, const QString&, const QString&, const QString&)), 
			this, SLOT(urlVisited(uint, const QString&, const QString&, const QString&)));
	...
	if(m_type == ListofBookmarks)
	{
		disconnect(m_iface, SIGNAL(bookmarkUpdated(uint, const QString&, const QString&, const QString&)), 
			this, SLOT(bookmarkUpdated(uint, const QString&, const QString&, const QString&)));
		disconnect(m_iface, SIGNAL(bookmarkRemoved(uint)), this, SLOT(bookmarkRemoved(uint)));
	}
...
BrowserItem* BrowserItemListModel::getItem(int id)
{
	QList<BrowserItem*>::iterator iter = m_items.begin();
...
void BrowserItemListModel::bookmarkRemoved(uint id)
{
...
void BrowserItemListModel::bookmarkUpdated(uint id, const QString &url, const QString &title, const QString &faviconUrl)
{
	...
	else
	{
      ...
      BrowserItem* item = m_items[m_items.length()-1];
...
void BrowserItemListModel::faviconUpdated(const QString &url)
{
	...
	{
		if(m_items[i]->faviconUri().contains(url))
		{
...
void BrowserItemListModel::urlVisited(uint id, const QString &url, const QString &title, const QString &faviconUrl)
{
	...
	else
	{
      ...
      BrowserItem *item = m_items[length - 1];
