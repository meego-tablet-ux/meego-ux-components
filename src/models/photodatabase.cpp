...
void PhotoDatabase::requestItem(int type, QString urn)
{
    ...
    {
        QObject::connect(watcher, SIGNAL(finished(QDBusPendingCallWatcher*)),
                     this, SLOT(trackerGetPhotoFinished(QDBusPendingCallWatcher*)));
    }
    else
    {
        QObject::connect(watcher, SIGNAL(finished(QDBusPendingCallWatcher*)),
...
void PhotoDatabase::trackerGetPhotoFinished(QDBusPendingCallWatcher *call)
{
    QDBusPendingReply<QVector<QStringList> > reply = *call;
    ...
    trackerAddItems(MediaItem::PhotoItem, info, true);
}

void PhotoDatabase::trackerGetAlbumFinished(QDBusPendingCallWatcher *call)
{
    QDBusPendingReply<QVector<QStringList> > reply = *call;
    ...
    QVector<QStringList> info = reply.value();
    trackerAddItems(MediaItem::PhotoAlbumItem, info, true);
}
