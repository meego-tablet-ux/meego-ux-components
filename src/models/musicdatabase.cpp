...
void MusicDatabase::processSong(MediaItem *item)
{
    ...
    {
        //qDebug() << "item " << item->m_title << " already has an artist item " << item->m_artist_urn.first();
       ...
       !mediaItemsUrnHash.contains(item->m_artist_urn.first()))
    {
        ...
        QDBusReply<QVector<QStringList> > reply =
                trackerinterface->callWithArgumentList(QDBus::BlockWithGui, QLatin1String("SparqlQuery"), argumentList);
       ...
       !mediaItemsUrnHash.contains(item->m_album_urn))
    {
        ...
        if (reply.isValid()&&!reply.value().isEmpty())
        {
            ...
            if(i != music.end())
            {
                ...
                if(!title.isEmpty())
                {
                    //qDebug() << "new MusicAlbumItem " << args;
            ...
            qDebug() << "Get Album QDBus Error: " << reply.error().message();
    }
...
{
    //qDebug() << "MusicDatabase::loadPlaylist";
    ...
    if (!reply.isValid()) {
        qDebug() << "Load Playlist QDBus Error: " << reply.error().message();
        return playlistItems;
    }

    QVector<QStringList> info = reply.value();
...
//        qDebug() << "startThumbnailerLoop targetting " << item->m_title << " " << item->m_type;
//        qDebug() << "ThumbnailerLoop items left: " << needThumbList.count();
        requestThumbnail(item);
    }
    else
    {
//        qDebug() << "startThumbnailerLoop done";
        thumbnailerTimer.stop();
    }
}

void MusicDatabase::requestItem(int type, QString urn)
{
    ...
    case MediaItem::SongItem:
        QObject::connect(watcher, SIGNAL(finished(QDBusPendingCallWatcher*)),
                     this, SLOT(trackerGetSongFinished(QDBusPendingCallWatcher*)));
        break;
    case MediaItem::MusicAlbumItem:
        QObject::connect(watcher, SIGNAL(finished(QDBusPendingCallWatcher*)),
                     this, SLOT(trackerGetAlbumFinished(QDBusPendingCallWatcher*)));
        break;
    case MediaItem::MusicArtistItem:
        QObject::connect(watcher, SIGNAL(finished(QDBusPendingCallWatcher*)),
                     this, SLOT(trackerGetArtistFinished(QDBusPendingCallWatcher*)));
        break;
    case MediaItem::MusicPlaylistItem:
        QObject::connect(watcher, SIGNAL(finished(QDBusPendingCallWatcher*)),
                     this, SLOT(trackerGetPlaylistFinished(QDBusPendingCallWatcher*)));
        break;
    }
}

void MusicDatabase::trackerGetSongFinished(QDBusPendingCallWatcher *call)
{
    QDBusPendingReply<QVector<QStringList> > reply = *call;

    /* no photos, either by error or end-of-query, quit */
    ...
    QVector<QStringList> info = reply.value();
    trackerAddItems(MediaItem::SongItem, info, true);
}

void MusicDatabase::trackerGetAlbumFinished(QDBusPendingCallWatcher *call)
{
    QDBusPendingReply<QVector<QStringList> > reply = *call;

    /* no albums, either by error or end-of-query, quit */
    if (reply.isError()||reply.value().isEmpty()) {
        if(reply.isError())
           qDebug() << "QDBus Error: " << reply.error().message();
        return;
    ...
    QVector<QStringList> info = reply.value();
    trackerAddItems(MediaItem::MusicAlbumItem, info, true);
}

void MusicDatabase::trackerGetArtistFinished(QDBusPendingCallWatcher *call)
{
    QDBusPendingReply<QVector<QStringList> > reply = *call;
    ...
    QVector<QStringList> info = reply.value();
    trackerAddItems(MediaItem::MusicArtistItem, info, true);
}

void MusicDatabase::trackerGetPlaylistFinished(QDBusPendingCallWatcher *call)
{
    QDBusPendingReply<QVector<QStringList> > reply = *call;
    ...
    QVector<QStringList> info = reply.value();
    trackerAddItems(MediaItem::MusicPlaylistItem, info, true);
}
